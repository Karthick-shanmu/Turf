...
const turfs = [
    {
        id: 'turf1',
        name: 'Natural Grass Turf',
        type: 'Natural Grass',
        price: 'Rs.500/hour',
        description: 'Premium natural grass surface with excellent drainage and maintenance. Perfect for traditional football experience.',
        image: 'https://images.unsplash.com/photo-1542857889-79e188618a1f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8Zm9vdGJhbGwlMjB0dXJmfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60'
    },
    {
        id: 'turf2',
        ...
        price: 'Rs.600/hour',
        description: 'High-quality artificial turf with shock absorption layer. All-weather playing surface with consistent bounce.',
        image: 'https://images.unsplash.com/photo-1596510913920-85d87a1800d2?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8YXJ0aWZpY2lhbCUyMHR1cmZ8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=500&q=60'
    },
    {
        id: 'turf3',
        name: 'Premium Artificial Turf',
        type: 'Premium Artificial',
        price: 'Rs.700/hour',
        description: 'Top-tier artificial turf with FIFA Quality Pro certification. Professional-grade surface for competitive play.',
        image: 'https://images.unsplash.com/photo-1518604666860-9ed391f76460?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTB8fGFydGlmaWNpYWwlMjB0dXJmfGVufDB8fDB8fHww&auto=format&fit=crop&w=500&q=60'
    }
];
...
function navigateTo(page) {
    currentPage = page
    selectedTurf = null // Reset turf selection when navigating
    renderPage()
    window.scrollTo(0, 0)
}
...
function updateBookingDisplay() {
    ...
    if (bookingSummary) {
        if (selectedSlots.length > 0) {
            ...
            const pricePerHour = selectedTurfData ? parseInt(selectedTurfData.price.match(/\d+/)[0]) : 500
            
            bookingSummary.style.display = 'block'
            bookingSummary.innerHTML = `
                <strong>Booking Summary:</strong><br>
                <strong>Selected Turf:</strong> ${selectedTurfData ? selectedTurfData.name : 'N/A'}<br>
                ...
                <strong>Rate:</strong> Rs.${pricePerHour} per hour<br>
                <strong>Total Cost:</strong> Rs.${selectedSlots.length * pricePerHour}
            `
        } else {
...
function submitBooking() {
    ...
    if (!date || !playerName || !contactNumber) {
    ...
    const booking = {
        ...
        date: date,
        turf: selectedTurf,
        turfName: selectedTurfData.name,
        slots: [...selectedSlots],
        playerName: playerName,
        contactNumber: contactNumber,
        teamSize: teamSize,
        totalCost: selectedSlots.length * pricePerHour,
    ...
    selectedSlots = []
    selectedTurf = null // Reset for next booking
    renderPage()
}
...
function renderHomePage() {
    // Clear and rebuild feature cards
    [featureCard1, featureCard2, featureCard3, featureCard4, featureCard5, featureCard6].forEach(card => {
    ...
    // Clear and rebuild features container
...
function renderAboutPage() {
    // Clear and rebuild about content
...
function renderContactPage() {
    // Clear and rebuild contact cards
    [contactCard1, contactCard2, contactCard3, contactCard4].forEach(card => {
        while (card.firstChild) {
            card.removeChild(card.firstChild)
        }
    })
    ...
    // Clear and rebuild contact info
    ...
    // Clear and rebuild form groups
    [formGroup1, formGroup2, formGroup3, formGroup4, formGroup5].forEach(group => {
    ...
    // Clear and rebuild contact form
...
function renderBookingPage() {
    if (!isLoggedIn) {
        // Clear and rebuild auth message
        while (authMessage.firstChild) {
            authMessage.removeChild(authMessage.firstChild)
        }
    ...
    // STEP 1: Show turf selection if no turf is selected
    if (!selectedTurf) {
        const selectTitle = document.createElement('h2')
        selectTitle.className = 'page-title'
        selectTitle.innerHTML = 'Select Your Turf'

        const turfGrid = document.createElement('div')
        turfGrid.className = 'turf-grid'

        turfs.forEach(turf => {
            const turfItem = document.createElement('div')
            turfItem.className = 'turf-item'
            ...
            info.className = 'turf-info'

            const name = document.createElement('div')
            name.className = 'turf-name'
            name.innerHTML = turf.name

            const type = document.createElement('div')
            type.className = 'turf-type'
            type.innerHTML = turf.type

            const price = document.createElement('div')
            price.className = 'turf-price'
            price.innerHTML = turf.price

            const desc = document.createElement('div')
            desc.className = 'turf-description'
            desc.innerHTML = turf.description
            ...
            turfItem.appendChild(img)
            turfItem.appendChild(info)

            // CLICK EVENT: Navigate to booking slots
            turfItem.onclick = () => {
                selectedTurf = turf.id
                renderBookingPage() // This will show Step 2
            }

            turfGrid.appendChild(turfItem)
        })

        pageContent.appendChild(selectTitle)
        pageContent.appendChild(turfGrid)
        return
    }

    // STEP 2: Show booking slots form (NO DROPDOWN - turf already selected)
    const selectedTurfData = turfs.find(t => t.id === selectedTurf)
    
    // Create booking page title with selected turf info
    const bookingPageTitle = document.createElement('h2')
    bookingPageTitle.className = 'page-title'
    bookingPageTitle.innerHTML = `Book ${selectedTurfData.name}`
    
    // Show selected turf info at top
    const selectedTurfInfo = document.createElement('div')
    selectedTurfInfo.className = 'feature-card'
    selectedTurfInfo.style.marginBottom = '2rem'
    selectedTurfInfo.innerHTML = `
        <h3>${selectedTurfData.name}</h3>
        <p><strong>Type:</strong> ${selectedTurfData.type}</p>
        <p><strong>Price:</strong> ${selectedTurfData.price}</p>
        <p>${selectedTurfData.description}</p>
    `

    // Back button to change turf
    const backButton = document.createElement('button')
    backButton.className = 'btn'
    backButton.innerHTML = '← Change Turf'
    ...
    bookingFormContainer.className = 'form-container'
    
    const bookingFormTitle = document.createElement('h3')
    bookingFormTitle.innerHTML = 'Select Your Booking Details'
    bookingFormTitle.style.textAlign = 'center'
    bookingFormTitle.style.marginBottom = '2rem'
    bookingFormTitle.style.color = '#2c3e50'
    ...
    // Time slots (NO DROPDOWN - directly show slots)
    ...
    nameLabel.innerHTML = 'Primary Contact Name *'
    const nameInput = document.createElement('input')
    nameInput.type = 'text'
    nameInput.id = 'playerName'
    nameInput.required = true
    nameInput.placeholder = 'Enter the main contact person\'s name'
    ...
    phoneLabel.innerHTML = 'Contact Number *'
    ...
    teamLabel.innerHTML = 'Team Size'
    const teamSelect = document.createElement('select')
    teamSelect.id = 'teamSize'
    
    const teamOptions = [
        {value: '5', text: '5 players (5-a-side)'},
        {value: '7', text: '7 players (7-a-side)'},
        {value: '11', text: '11 players (Full team)', selected: true},
    ...
    submitButton.innerHTML = 'Confirm Booking (0 slots)'
    submitButton.id = 'bookingBtn'
    ...
    bookingForm.appendChild(nameGroup)
    bookingForm.appendChild(phoneGroup)
    bookingForm.appendChild(teamGroup)
    ...
    bookingFormContainer.appendChild(bookingSummary)
    
    // Add everything to page
    pageContent.appendChild(backButton)
    pageContent.appendChild(bookingPageTitle)
    pageContent.appendChild(selectedTurfInfo)
    pageContent.appendChild(bookingFormContainer)
    
    // Initialize slots and form functionality
    renderSlots()
    updateBookingDisplay()
...
function renderLoginPage() {
    ...
    // Clear and rebuild login groups
    [loginGroup1, loginGroup2].forEach(group => {
        while (group.firstChild) {
            group.removeChild(group.firstChild)
        }
    })
    ...
    // Clear and rebuild login form container
    ...
    // Clear and rebuild login container
